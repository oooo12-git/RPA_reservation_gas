...
function edit(e) {
  ...
  let PRICE_KO_COLUMN = 24; // AB열
  let PRICE_EN_COLUMN = 25; // AC열
  let STUDIO_COLUMN = 26; // AD열
  let SEND_MAIL_COLUMN = 27; // AE열
  let CONFIRM_COLUMN = 28; // AF열
  let EVENT_ID_COLUMN = 29; // AG열
  ...
  if (sheetName === 'info' && editedColumn === CALCULATE_BUTTON_COLUMN) { // T열은 20번째 열
    ...
    if (value === true) {
      ...
      if (!isNaN(numberOfPeople) && numberOfPeople !== "") {
        ...
        sheet.getRange(row, PRICE_KO_COLUMN).setValue('');
        sheet.getRange(row, PRICE_EN_COLUMN).setValue('');
      } else {
        // 숫자가 아닌 경우 "기입필요" 입력
      ...
      calculateAndSetPrice(sheet, row, PRICE_KO_COLUMN, PRICE_EN_COLUMN,couple_profile, group_profile, 
        individual_1st, individual_1st_hm, 
        individual_2nd, individual_2nd_hm,
        individual_3rd, individual_3rd_hm,
  ...
  if (sheetName === 'info' && editedColumn === SEND_MAIL_COLUMN && value == "Send!") {
    ...
    // {values: ["A", "B", "C"]} 형태로 전달.
    addCalendarSendMailAddContact({values: rowValues, row: row},row, name, phoneNumber, email, date_of_shooting, numberOfPeople, studio, sendMail, couple_profile, group_profile, individual_1st, individual_2nd, individual_3rd, EVENT_ID_COLUMN, DEPOSIT_COLUMN, DEPOSIT_DOLLAR_COLUMN, PRICE_KO_COLUMN, PRICE_EN_COLUMN);
  }

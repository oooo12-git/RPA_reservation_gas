function formSubmit_selection(e) {
  ...
  let newRecord = selectionSheet
    .getRange(lastRow, 1, 1, selectionSheet.getLastColumn())
    .getValues()[0];
  ...
  if (
    selected_picture_number === null ||
    selected_picture_number === undefined
  ) {
    selected_picture_count = 0;
  } else {
    // 숫자를 문자열로 변환 후 처리
    selected_picture_count = String(selected_picture_number)
      .trim()
      .split(/\s+/).length;
  }
  ...
  let infoData = infoSheet
    .getRange(2, 7, infoSheet.getLastRow() - 1, 1)
    .getValues(); // 7번째 열(이메일) 데이터
  ...
  for (let i = 0; i < infoData.length; i++) {
    if (infoData[i][0] === selection_email) {
      ...
      infoSheet
        ...
        .setValue(selected_picture_number);
      infoSheet
      ...
      // ADJUST_CALENDAR_ADD_COLUMN 체크박스 체크
      infoSheet
        .getRange(row, columns.ADJUST_CALENDAR_ADD_COLUMN)
        .setValue(true);
      ...
      Logger.log(
        "데이터 업데이트 완료 - 행: " +
          row +
          ", 선택된 사진 번호: " +
          selected_picture_number +
          ", 선택된 날짜: " +
          selected_date +
          ", 마감 날짜: " +
          due_date +
          ", 선택된 사진 개수: " +
          selected_picture_count
      );
      break;
    }
  }

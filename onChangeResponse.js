function onChangeResponse(e) {
    // 스프레드시트와 response 시트 가져오기
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    var responseSheet = ss.getSheetByName('response');
    if (!responseSheet) {
      Logger.log("response 시트를 찾을 수 없습니다.");
      return;
    }
    
    var currentLastRow = responseSheet.getLastRow();
    var props = PropertiesService.getScriptProperties();
    var lastRowStored = props.getProperty("LAST_ROW_RESPONSE");
    
    if (lastRowStored) {
      lastRowStored = parseInt(lastRowStored, 10);
    } else {
      // 최초 실행 시 현재 마지막 행 번호 저장 후 종료
      props.setProperty("LAST_ROW_RESPONSE", currentLastRow.toString());
      Logger.log("LAST_ROW_RESPONSE 초기화: " + currentLastRow);
      return;
    }
    
    // 새 행이 추가되었는지 확인 (여러 행이 추가될 수도 있으므로 반복문으로 처리)
    if (currentLastRow > lastRowStored) {
      Logger.log("새 행 추가 감지: 이전 행 번호 " + lastRowStored + ", 현재 행 번호 " + currentLastRow);
      
      for (var row = lastRowStored + 1; row <= currentLastRow; row++) {
        // 신규 행 데이터 가져오기
        var newRecord = responseSheet.getRange(row, 1, 1, responseSheet.getLastColumn()).getValues()[0];
        var fakeEvent = { values: newRecord };
        try {
          responseToInfo(fakeEvent);
          Logger.log("행 " + row + " 처리 완료");
        } catch (error) {
          Logger.log("행 " + row + " 처리 중 오류 발생: " + error);
        }
      }
    }
    ...
    // 마지막 행 번호 업데이트
    props.setProperty("LAST_ROW_RESPONSE", currentLastRow.toString());
    Logger.log("LAST_ROW_RESPONSE 업데이트: " + currentLastRow);
  }

  function responseToInfo(e) {

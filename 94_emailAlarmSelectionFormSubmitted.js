function emailAlarmSelectionFormSubmitted(name, time, due_date) {
    Logger.log('emailAlarmSelectionFormSubmitted 함수 실행됨');
        // 시간 형식 변환
        const date = new Date(time);
        const koreanTime = `${date.getFullYear()}년 ${date.getMonth() + 1}월 ${date.getDate()}일 ${date.getHours()}시 ${date.getMinutes()}분`;
        
        // 이메일 제목과 본문 설정
        let subject = '보정 신청 알림 ' + name + '님 ' + koreanTime;
        let body = `${name}님이 보정 신청하셨습니다. \n` + 
                    '신청 시간: ' + koreanTime + 
                    '\n보정 기한 : ' + due_date + 
                    '\n확인해 주세요.';
